...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    // Handle input right away. None of the code of BeginWindows() is relying on scrolling position before calling Scrollbar().
    bool held = false;
    bool hovered = false;
    ...
    if (held && allow_interaction && grab_h_norm < 1.0f)
    {
        ...
        // It is ok to modify Scroll here because we are being called in BeginWindows() after the calculation of ContentSize and before setting up our starting position
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    g.NextWindowData.ClearFlags(); // We behave like BeginWindows() and need to consume those values
    if (window->SkipItems)
        return false;
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    // FIXME: This might be moved to BeginWindows() or at least around the same spot where Tooltips and other Popups are calling FindBestWindowPosForPopupEx()?
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    // If a menu with same the ID was already submitted, we will append to it, matching the behavior of BeginWindows().
    ...
    // The reference position stored in popup_pos will be used by BeginWindows() to find a suitable position for the child menu,
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        // For ChildMenu, the popup position will be overwritten by the call to FindBestWindowPosForPopup() in BeginWindows()
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    // Forward Hovered state so IsItemHovered() after BeginWindows() can work (even though we are technically hovering our parent)
