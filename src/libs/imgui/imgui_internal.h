...
// Note that Max is exclusive, so perhaps should be using a BeginWindows/End convention.
struct ImGuiListClipperRange
{
    int     Min;
    int     Max;
    bool    PosToIndexConvert;      // BeginWindows/End are absolute position (will be converted to indices later)
...
// List of colors that are stored at the time of BeginWindows() into Docked Windows.
...
struct ImGuiContext
{
    ...
    // Shared stacks
    ImVector<ImGuiColorMod>     ColorStack;                     // Stack for PushStyleColor()/PopStyleColor() - inherited by BeginWindows()
    ImVector<ImGuiStyleMod>     StyleVarStack;                  // Stack for PushStyleVar()/PopStyleVar() - inherited by BeginWindows()
    ImVector<ImFont*>           FontStack;                      // Stack for PushFont()/PopFont() - inherited by BeginWindows()
    ImVector<ImGuiID>           FocusScopeStack;                // Stack for PushFocusScope()/PopFocusScope() - inherited by BeginChild(), pushed into by BeginWindows()
    ImVector<ImGuiItemFlags>    ItemFlagsStack;                 // Stack for PushItemFlag()/PopItemFlag() - inherited by BeginWindows()
    ImVector<ImGuiGroupData>    GroupStack;                     // Stack for BeginGroup()/EndGroup() - not inherited by BeginWindows()
    ...
    ImGuiViewportP*         CurrentViewport;                    // We track changes of viewport (happening in BeginWindows) so we can call Platform_OnChangedViewport()
...
// (This doesn't need a constructor because we zero-clear it as part of ImGuiWindow and all frame-temporary data are setup on BeginWindows)
struct IMGUI_API ImGuiWindowTempData
{
    // Layout
    ImVec2                  CursorPos;              // Current emitting position, in absolute coordinates.
    ImVec2                  CursorPosPrevLine;
    ImVec2                  CursorStartPos;         // Initial position after BeginWindows(), generally ~ window position + WindowPadding.
    ...
    ImGuiLayoutType         ParentLayoutType;       // Layout type of parent window at the time of BeginWindows()
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImGuiViewportP*         Viewport;                           // Always set in BeginWindows(). Inactive windows may have a NULL value here if their viewport was discarded.
    ...
    ImVec2                  WindowPadding;                      // Window padding at the time of BeginWindows().
    float                   WindowRounding;                     // Window rounding at the time of BeginWindows(). May be clamped lower to avoid rendering artifacts with title bar, menu bar etc.
    float                   WindowBorderSize;                   // Window border size at the time of BeginWindows().
    float                   DecoOuterSizeX1, DecoOuterSizeY1;   // Left/Up offsets. Sum of non-scrolling outer decorations (X1 generally == 0.0f. Y1 generally = TitleBarHeight + MenuBarHeight). Locked during BeginWindows().
    ...
    bool                    ViewportOwned;
    bool                    Active;                             // Set to true on BeginWindows(), unless Collapsed
    bool                    WasActive;
    ...
    short                   BeginCount;                         // Number of Begin() during the current frame (generally 0 or 1, 1+ if appending via multiple BeginWindows/End pairs)
    short                   BeginCountPreviousFrame;            // Number of BeginWindows() during the previous frame
    short                   BeginOrderWithinParent;             // BeginWindows() order within immediate parent window, if we are a child window. Otherwise 0.
    short                   BeginOrderWithinContext;            // BeginWindows() order within entire imgui context. This is mostly used for debugging submission order related issues.
    ...
    ImRect                  OuterRectClipped;                   // == Window->Rect() just after setup in BeginWindows(). == window->Rect() for root window.
    ...
    ImGuiID                 NavRootFocusScopeId;                // Focus Scope ID at the time of BeginWindows()
...
//#endif
    ...
    // There is one per window (automatically set in BeginWindows), but:
