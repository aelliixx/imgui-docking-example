...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    // SetupImGui memory buffer (clear data if columns count changed)
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    // [Part 6] SetupImGui final position, offset, skip/clip states and clipping rectangles, detect hovered column
    ...
    // SetupImGui window's WorkRect.Max.y for GetContentRegionAvail(). Other values will be updated in each TableBeginCell() call.
    ...
    // [Part 13] SetupImGui inner window decoration size (for scrolling / nav tracking to properly take account of frozen rows/columns)
...
void    ImGui::EndTable()
{
    ...
    // SetupImGui inner scrolling range
